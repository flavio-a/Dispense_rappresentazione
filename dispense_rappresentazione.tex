...
\author{Matematici, fisici e informatico del secondo anno, a.a. 2017/2018}
\date{\today}
...
\begin{defn}[Centralizzatore di un elemento]
	Sia $G$ un gruppo e $g\in G$ un elemento. Si chiama \emph{centralizzatore} di $g$ e si indica con $Z(g)$ il sottoinsieme di elementi di $G$ che commutano con $g$:
	\[Z(g)=\{ h\in G\ |\ hg=gh \}\]
	\`E immediato verificare che $Z(g)$ è un sottogruppo di $G$ per ogni $g$.
\end{defn}
...
\end{exmp}
\begin{note}
	Nel seguito indicherò con $\rho^*\boxtimes\sigma$ anche la rappresentazione su $\Hom(V, W)$. Dal contesto dovrebbe sempre essere chiaro quale sia il codominio della rappresentazione.
\end{note}
...
\end{rem}
$ $\\
Sia $Irr(G)=\{\rho \ |\ \rho\text{ è una rappresentazione irriducibile di }G \}$
\begin{thm}
	Al variare di $\rho$ rappresentazione irriducibile di $G$
	\[
	\hat{\rho}_{reg} \iso \sum_{\rho\in Irr(G)}\rho\boxtimes\rho^*
	\]
\end{thm}
\begin{proof}
	Sia $\rho:G\to GL(V_\rho)$ irriducibile. Definisco $T_\rho:\C[G]\to\End(V_\rho)$ come $T_\rho(e_g)=\rho(g)$ estesa per linearità. Verifico che $T_\rho$ è un omomorfismo di rappresentazioni con la rappresentazione $\rho\boxtimes\rho^*:G\to GL(V\otimes V^*)$, ricordando che $V\otimes V^*\iso\End(V)$:
	\begin{align*}
		T_\rho(\hat{\rho}_{reg}(g_1, g_2)e_h) &= T_\rho(e_{g_1 h g_2^{-1}}) =\\
		&= \rho(g_1 h g_2^{-1}) =\\
		&= \rho(g_1)\rho(h)\rho(g_2)^{-1}\\
		&= \rho\boxtimes\rho^*(g_1, g_2)(\rho(h))=\rho\boxtimes\rho^*(g_1, g_2)T_\rho(e_h)
	\end{align*}
	Dato che $\rho$ è irriducibile lo è anche $\rho\boxtimes\rho^*$, dunque per il lemma di Schur $T_\rho$ è costantemente nulla oppure suriettiva. $T_\rho(e_e)=\rho(e)=\Id\neq0$, quindi $T_\rho$ è suriettiva. Se considero $\Ker(T_\rho)^\perp$ $G$-stabile, la restrizione $\left.T_\rho\right|_{\Ker(T_\rho)^\perp}$ è un'isomorfismo di rappresentazioni, quindi $\hat{\rho}_{reg}$ ha una sottorappresentazione isomorfa a $\rho\boxtimes\rho^*$.\\
	Considero due rappresentazioni irriducibili e isomorfe $\rho\boxtimes\sigma \iso \rho'\boxtimes\sigma'$ di $G\times G$. Supponiamo per assurdo che $\rho\noniso\rho'$. Allora $deg(\rho)\cdot\rho\iso\left.\rho\boxtimes\sigma\right|_{G\times\lbrace1\rbrace}\iso\left.\rho'\boxtimes\sigma'\right|_{G\times\lbrace1\rbrace}\iso deg(\rho')\cdot\rho'$, ma questo è assurdo perché $\rho, \rho'$ sono irriducibili non isomorfe. Quindi $\rho\iso\rho'$, e similmente $\sigma\iso\sigma'$.\\
	Se $\rho, \sigma$ sono due rappresentazioni irriducibili di $G$ non isomorfe allora $\rho\boxtimes\rho^* \noniso \sigma\boxtimes\sigma^*$, quindi nella somma
	\[
	\sum_{\rho\in Irr(G)} \rho\boxtimes\rho^*
	\]
	tutti i termini sono non isomorfi. Da un rapido calcolo di grado si ottiene
	\[
	deg(\hat{\rho}_{reg}) = |G| = \sum_{\rho\in Irr(G)} deg(\rho)^2 = \sum_{\rho\in Irr(G)} \dim(V_\rho)^2 = \sum_{\rho\in Irr(G)} \dim(\End(V_\rho)) = \sum_{\rho\in Irr(G)} deg(\rho\boxtimes\rho^*)
	\]
	quindi $\hat{\rho}_{reg}$ non contiene altre sottorappresentazioni.
\end{proof}
\begin{cor}
	\[
	\C[G]\iso \bigoplus\limits_{\rho\in Irr(G)} \End(V_\rho)
	\]
\end{cor}
$ $\\
Aggiungo una modo alternativo per trovare la sottorappresentazione isomorfa a $\rho\boxtimes\rho^*$ nel teorema precedente.
\begin{proof}[Dimostrazione alternativa]
	Consideriamo $\C^G=\lbrace f:G\to\C\rbrace$. C'è un isomorfismo canonico tra $\C^G$ e $\C[G]$ che manda $f\mapsto \sum_g f(g)e_g$; è facile verificare che tale isomorfismo è anche di rappresentazioni tra la rappresentazione $\hat{\rho}_{reg}$ su $\C[G]$ e la rappresentazione su $\C^G$ tale che $(g_1, g_2)f(h)=f(g_1^{-1}hg_2)$. Data $\rho$ irriducibile voglio trovare un isomorfismo tra $\End(V_\rho)$ e un sottospazio $G\times G$-stabile di $\C[G]$, ma grazie a due isomorfismi posso cercarlo in realtà tra $V_\rho\otimes V_\rho^*$ e $\C^G$.\\
	Sia $\lbrace v_i\rbrace_{i\in I}$ una base di $V$, e sia $\lbrace v_i^*\rbrace_{i\in I}$ la base duale.
	Definisco $S_\rho: V_\rho\otimes V_\rho^*\to\C^G$ tale che $S_\rho(v_i\otimes v_j^*)(g)=v_j^*(\rho(g)v_i)$ ed esteso per linearità. Verifico che $S_\rho$ è un omomorfismo di rappresentazioni:
	\begin{align*}
		S_\rho\left( \rho\boxtimes\rho^*(g_1, g_2)\sum_{(i, j)\in I^2} a_{ij}v_i\otimes v^*_j \right)(g) &= 	\sum_{(i, j)\in I^2} a_{ij} S_\rho(\rho(g_1)v_i\otimes\rho^*(g_2)v^*_j)(g) =\\
		= \sum_{(i, j)\in I^2} a_{ij} \rho^*(g_2)v^*_j(\rho(g)\rho(g_1)v_i) &= \sum_{(i, j)\in I^2} a_{ij} v^*_j(\rho(g_2)^{-1}\rho(g)\rho(g_1)v_i) = \\
		= S_\rho\left(\sum_{(i, j)\in I^2} a_{ij} v^*_j\otimes v_i\right)(g_2^{-1} g g_1) &= (g_1, g_2)S_\rho\left( \sum_{(i, j)\in I^2} a_{ij}v_i\otimes v^*_j \right)(g)
	\end{align*}
	Dato che $\rho$ è irriducibile lo è anche $\rho\boxtimes\rho^*$, quindi per il lemma di Schur e visto che $S_\rho(v_i\otimes v^*_i)(e_G)= v^*_i(\rho(e_G)v_i) = v^*_i(v_i) = 1 \neq 0$ deve essere $S_\rho$ iniettiva, quindi $S_\rho$ è un isomorfismo tra $V_\rho\otimes V_\rho^*$ e $\C^G$.
\end{proof}

Utilizzando il teorema appena dimostrato diamo una dimostrazione dell'ortogonalità della tavola dei caratteri per colonne.
\begin{thm}
	Comunque scelti $g, h\in G$ vale
	\[
	\sum_{\rho\in Irr(G)} \chi_\rho(g) \overline{\chi_\rho(h)} =
	\begin{cases}
		0 \quad \text{se } g, h \text{ sono coniugati} \\
		|Z(g)| \quad \text{altrimenti}
	\end{cases}
	\]
	dove $Z(g)$ è il centralizzatore di $g$.
\end{thm}
\begin{proof}
	\begin{align*}
		\sum_{\rho\in Irr(G)} \chi_\rho(g) \overline{\chi_\rho(h)} &= \sum_{\rho\in Irr(G)} \chi_\rho(g) \chi_{\rho^*}(h) = \\
		&= \sum_{\rho\in Irr(G)} \chi_{\rho\boxtimes\rho^*}(g, h) = \\
		&= \chi_{\hat{\rho}_{reg}}(g, h)
	\end{align*}
	Dato che $\hat{\rho}_{reg}$ è una rappresentazione per permutazione su $\C[G]$ le immagini degli elementi del gruppo sono matrici di permutazione, quindi $\chi_{\hat{\rho}_{reg}}(g_1, g_2)$ è il numero di punti fissi di $\hat{\rho}_{reg}(g_1, g_2)$, ovvero $|\{h\in G\ |\ g_1 h g_2^{-1} = h \}|$.\\
	Un $\overline{h}\in G$ tale che $g_1 \overline{h} g_2^{-1} = \overline{h}$ esiste se e solo se $\overline{h}^{-1} g_1 \overline{h} = g_2$, ovvero se $g_1, g_2$ sono coniugati. Da questo segue subito che $g_1, g_2$ non sono coniugati $\chi_{\hat{\rho}_{reg}}(g_1, g_2) = |\{h\in G\ |\ g_1 h g_2^{-1} = h \}| = 0$. Se invece $g_1, g_2$ sono coniugati, ovvero se esiste $\overline{h}$, vale che
	\[
		\{h\in G\ |\ g_1 h g_2^{-1} = h \} = \{h\in G\ |\ g_1 = h g_2 h^{-1} \} =  \{h\in G\ |\ g_1 = h \overline{h} g_1 \overline{h}^{-1} h^{-1} \}
	\]
	Quindi l'applicazione $h\mapsto\overline{h} h$ è una biezione tra $Z(g)$ e $\{h\in G\ |\ g_1 h g_2^{-1} = h \}$, perciò questi due insiemi hanno la stessa cardinalità.
\end{proof}
\begin{cor}
	Le colonne della tabella dei caratteri sono ortogonali.
\end{cor}
\begin{proof}
	Ogni colonna della tabella dei caratteri corrisponde ad una diversa classe di coniugo quindi, detti $g$ e $h$ rappresentanti di due colonne diverse, $g$ e $h$ non sono coniugati e quindi
	\[
		\sum_{\rho\in Irr(G)} \chi_\rho(g) \overline{\chi_\rho(h)} = 0
	\]
	dove la prima somma è proprio il prodotto hermitiano standard di $\C^n$ tra le due colonne di $g$ e $h$.
\end{proof}
