...
\begin{thm}
	...
	\[
	\hat{\rho}_{reg} \iso \sum_{\rho}\rho\boxtimes\rho^*
	\]
\end{thm}
\begin{proof}
	...
	\[
	\sum_\rho \rho\boxtimes\rho^*
	\]
	tutti i termini sono non isomorfi. Da un rapido calcolo di grado si ottiene
	\[
	deg(\hat{\rho}_{reg}) = |G| = \sum_\rho deg(\rho)^2 = \sum_\rho \dim(V_\rho)^2 = \sum_\rho \dim(\End(V_\rho)) = \sum_\rho deg(\rho\boxtimes\rho^*)
	\]
	quindi $\hat{\rho}_{reg}$ non contiene altre sottorappresentazioni.
\end{proof}
\begin{cor}
	\[
	\C[G]\iso \bigoplus\limits_\rho \End(V_\rho)
	\]
\end{cor}
$ $\\
